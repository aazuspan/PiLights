...
class Fireflies(Visualization):
    ...
    def render(self):
        ...
        # Set starting color
        faded_color = self.min_color
        
        # Fade firefly color in
        for i in range(0, self.fade_in_length):
            faded_color = utils.interpolate_color(self.min_color, self.max_color, i/self.fade_in_length)
            self.pixels[firefly_index] = utils.floatcolor2intcolor(faded_color)
            self.pixels.show()
        
        # Fade firefly color out
        for i in range(0, self.fade_out_length):
            faded_color = utils.interpolate_color(self.max_color, self.min_color, i/self.fade_out_length)
            self.pixels[firefly_index] = utils.floatcolor2intcolor(faded_color)
            self.pixels.show()
